<div class="space-y-8">
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach ($facilities as $facility)
                        <label for="{{ $facility->code }}" class="bg-gray-100 py-2 px-4 rounded-tr-xl rounded-bl-xl flex items-center gap-1 cursor-pointer">
                            <input wire:model="selectedFacilities.{{ $facility->code }}" value="{{ $facility->code }}" class="checkbox cursor-pointer" type="checkbox" name="selectedFacilities[]" id="{{ $facility->code }}">
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @room:created.window="open = true" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
                ...
                <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                    <div class="text-center space-y-4">
                        ...
                        <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                            New room have been added! You can check it on the room dashboard!
                        </p>
                    </div>
    ...
    <script>
        ClassicEditor
            ...
            .then(editor => {
                ...
                window.addEventListener('room:created', () => {
