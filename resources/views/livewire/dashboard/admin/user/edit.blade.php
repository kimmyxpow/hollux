<div x-data="{ open: false }">
    <button class="btn btn-sm" x-on:click='open = true'>Edit</button>
    <div x-show="open" @user:edited.window="open = false" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            ...
            <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-2xl w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                ...
                <div class="grid sm:grid-cols-2 gap-4 items-start">
                    ...
                    <div class="form-control">
                        ...
                        <input type="password" id="password" name="password" wire:model='password' class="input">
                        @error('password')
                            <span class="invalid">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-error="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress" class="space-y-4">
                    <div class="form-control">
                        <label for="avatar" class="btn w-full text-center">Change Avatar</label>
