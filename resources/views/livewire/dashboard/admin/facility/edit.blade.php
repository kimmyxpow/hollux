<div class="space-y-8">
    ...
    <form class="space-y-4" action="#" method="POST" wire:submit.prevent='update'>
        ...
        <button type="submit" class="btn" wire:loading.attr="disabled">Edit</button>
    </form>
    <div x-data="{ open: false }">
        <div x-show="open" @facility:edited.window="open = true" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
                ...
                <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                    <div class="text-center space-y-4">
                        ...
                        <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                            Facility data edited successfully! You can see it on the facility dashboard!
                        </p>
                    </div>
                    <div class="flex space-x-2 justify-center">
                        <a href="{{ route('dashboard.admin.facilities.index') }}" type="button" class="btn">
                            View List
                        </a>
